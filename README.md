...
Испоьзуются следующие таблицы для хранения данных:
<ul>
...
## Основной загрузчик
Родительским абстрактным классом для загрузки всех четырёх типов данных находится в файле <i>app/Classes/BaseLoader.php</i>, который имеет абстрактных метод load.  
...
Так как достоверно не известно, какие поля в БД должны быть уникальными, при запуске консольной команды происходит отчистка таблиц базы данных.  
Метод getAdditionalQueryParams класса BaseLoader переопределяется только в классе StockLoader, так как там нужен только параметр dateFrom с датой равной сегдняшней дате.
